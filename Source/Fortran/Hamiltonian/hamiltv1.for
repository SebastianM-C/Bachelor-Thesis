...
c     implicit double precision (a-h,o-z)
 ...
 3001 format(' i j i1 j1 i2 j2 a =',6i4,f8.3)
      ...
      open(unit=20,file='hamilt.out',status='new')
...
c        enddo
        ...
        write(30,2003)j,dd(j)
          if(j.eq.1) then
          m2=m2+1
          write(50,2003) m2,dd(j)
          go to 99
          endif
          dif1=dd(j+1)-dd(j)
          dif=dd(j)-dd(j-1)
          if(dif.le.0.0005) then
           m1=m1+1
           write(40,2003) m1,dd(j)
          endif
          if(dif.gt.0.0005.and.dif1.gt.0.0005) then
           call select(nn,j,j,2,xx)
            if(iii.eq.1) then
             m2=m2+1
             write(50,2003) m2,dd(j)
            endif
            if(iii.eq.2) then
             m3=m3+1
             write(60,2003) m3,dd(j)
            endif
          endif
 99        continue
        call wmat(20,nn,j,j,2,xx)
      end do
      close(unit=10)
      close(unit=20)
      close(unit=30)
